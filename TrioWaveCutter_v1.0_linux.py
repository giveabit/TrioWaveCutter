...
def scanAudio(data, start, end):
    ...
    while not audio and not end > len(data):
        if start % 10 == 0:
            ...
            # print('-', bytesToTime(start), bytesToTime(end))
        meanTest, foo = meanLoudness(data, start, end)
        if meanTest > cutOn:
            audio = True
            # input(audio)
            starts.append(start-lenght)
            out = '\t\t\t'+str(bytesToTime(start - lenght))+' ms\r'
        ...
        while audio and not end > len(data):
            ...
            if counter == 1000:
                ...
                if firstRun:
                    ...
                    out = '\t\t\t' + str(bytesToTime(starts[-1]))+ ' ms \t...\t' + str(bytesToTime(startOld)) + ' ms\n'
                    sys.stdout.write(out)
            if skipAhead:
                start += 1000*lenght
                end = start + lenght
...
def writeRoutine(data, starts, ends, userArgs):
    audio = zip(starts, ends)
    sequence = []
    instruments = {'g': 'guitar', 'b': 'bass', 'd': 'drums'}

    for char in userArgs:
...
for fileName in fileList:
    ...
    cutOff = meanFloor # must not be negative !!!
    ...
    sys.stdout.write('\n\t\t\t[x] writing files.')
    writeRoutine(data, starts, ends, userArgs)
